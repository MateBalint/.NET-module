    ...
    public class InMemoryRepository : IApplicationRepository
    {
        ...
        public int AmountOfApplicationAfter(DateTime date)
        {
            int numberOfApplications =
                _applications.Count(applicationDate => applicationDate.ApplicationDate > date);
        ...
        {
            IEnumerable<Mentor> mentorsWithFavoriteLanguage =
                from mentor in _mentors
                where mentor.ProgrammingLanguage == language
                select mentor;
            return mentorsWithFavoriteLanguage;
        }

        /// <inheritdoc/>
        public IEnumerable<Applicant> GetApplicantsOf(string contactMentorName)
        {
            IEnumerable<Applicant> applicantsOf =
                from applicant in _applications
                where (applicant.Mentor.FirstName + " " + applicant.Mentor.LastName) == contactMentorName
